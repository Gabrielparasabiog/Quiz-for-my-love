...
The current bank combines the original test bank with marked image questions and the supplied `Construction-Concepts-and-Terms-Compilation-Apr-2025.pdf`. Repeated prompts are removed by normalized wording plus a maintained semantic duplicate list; the app validates IDs, categories, prompts, and choice uniqueness at startup and shows a friendly content-check screen rather than starting with invalid content. Run the checks above before committing a replacement bank.
